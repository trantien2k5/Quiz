...
**Mục chưa có chức năng/data thật vẫn hiện trong UI** kèm `devRow()`/`devBadge()` (badge cam "🚧 Đang phát triển") thay vì ẩn đi — gồm: Avatar/Rank, AI Insight thật (hiện là heuristic `generateInsights()`), tag Part/từ loại/ngữ pháp/CEFR/độ khó/loại câu/nguồn (đã hỏi user, xác nhận bỏ qua — KHÔNG tự thêm field tag câu hỏi nếu chưa hỏi lại), AI dự đoán sẽ quên (khác forgetting curve công thức ở trên — đây là dự đoán bằng AI thật, chưa làm), Precision/Recall/Confidence/Fatigue Score, Calendar/Replay/Ghi chú, PDF/Excel/Đồng bộ/Sao lưu. Phải hỏi user trước khi build thật bất kỳ mục nào ở đây.
...
**Forgetting curve / lịch ôn tập giữa các ngày (tab Ghi nhớ)**: khác với BKT `pL` (chỉ phản ánh mức thuộc tại lần ôn gần nhất, không tự giảm theo thời gian), `getQuestionRetentionInfo(s)` (src/features/history/history.js) áp công thức Ebbinghaus `retention = pL * e^(-daysSince/stability)` để suy ra mức nhớ THỰC TẾ tại thời điểm hiện tại (giảm dần nếu lâu chưa ôn lại). `getMemoryStabilityDays(pL, reviewCount)` — pL cao + ôn nhiều lần → stability (số ngày nhớ được) lớn hơn. `s.lastReviewed` (field mới trong `quiz_q_stats[id]`, ghi ở `trackQuestionResult()` mỗi lần làm câu đó) — data cũ chưa có field này thì fallback `firstSeen`. `getMemoryForgettingStats()` là nguồn tính duy nhất (đọc toàn bộ `quiz_q_stats`, bỏ qua câu `reviewCount===0` vì là câu MỚI chưa học chứ không phải đang quên, và câu đã bị xoá khỏi bộ đề) → trả về danh sách `due` (retention rơi dưới `MEMORY_REVIEW_THRESHOLD=0.7`, sort quên nhiều nhất lên đầu) + `avgStability`. Nút "🔁 Ôn N câu cần ôn hôm nay" (`reviewQuestionsDueToday()`) tái dùng đúng pattern `retryAllWrongQuestions()` — gọi `startPractice({id,name,questions})` với set giả, không qua `saveSet()`.

**"Gợi ý hôm nay" (Dashboard, đầu tab Tổng quan, dưới Level card)**: `getTodayRecommendation(history)` (src/features/history/history.js) — KHÁC `generateInsights()`/`generateImprovementItems()` (chỉ liệt kê số liệu mô tả, không có hành động) — đây là 1 quy tắc quyết định chọn ĐÚNG 1 hành động ưu tiên nhất để hiện kèm nút bấm. Thứ tự ưu tiên cố định (cố ý, không phải ngẫu nhiên): (1) `getMemoryForgettingStats().due.length >= 3` → quên nhiều câu, ôn ngay (mất công học lại từ đầu nếu để quên) → nút gọi `reviewQuestionsDueToday()`; (2) `getFixStatus(history).unresolved.length > 0` → còn câu sai chưa sửa được → `retryAllWrongQuestions()`; (3) chủ đề yếu nhất (`computeSetStats()`, accuracy <70%, ≥5 câu) → `startPractice(setId)`; (4) chưa đạt Nhiệm vụ hôm nay (`getDailyQuestProgress()`) → `navTo('library')`; (5) còn ít câu cần ôn (due <3) → vẫn gợi ý ôn; (6) không còn gì → câu khen tích cực, không có nút. Đổi thứ tự ưu tiên thì sửa Ở ĐÂY (1 chỗ duy nhất).
...
- FAB "+" ở Home/Library gọi trực tiếp `openImportText()` (vào thẳng màn Dán văn bản, KHÔNG qua modal chọn tab nữa — bỏ bước "Bằng AI" khỏi flow này theo yêu cầu user). `modal-ai-create` (2 tab `.aqc-tabs`: "Bằng AI" / "Dán văn bản" qua `goToImportTextFromAiCreate()` → `openImportText(_appendToSetId)`) vẫn còn dùng ở 2 nơi khác: menu "Thêm câu hỏi" trong set (`setActionAddQuestions()`, src/features/library/library.js) và gợi ý từ "Phân tích AI" (`quickCreateFromAnalysis()`, src/features/ai/ai.js) — cả 2 vẫn cần tab AI nên KHÔNG đổi. `showAICreate()` luôn reset về tab 'ai' khi mở. `.aqc-tabs`/`.aqc-tab`/`.aqc-page` định nghĩa ở src/features/editor/editor.css nhưng dùng chung global (không scoped theo screen)
...
- **"Dán tách" (3 ô riêng)**: nút `switchImportPasteMode('split')` cạnh ô dán gộp → `#import-text-split-step` (3 textarea riêng: câu hỏi+4 lựa chọn / đáp án đúng / giải thích, dùng khi 3 phần lấy từ 3 nguồn khác nhau, ví dụ câu hỏi từ file Word, đáp án từ answer key, giải thích từ AI). `parseImportTextSplit()`: ô 1 tái dùng `parseAzotaText()` (chỉ lấy `.text`/`.options`, bỏ qua `.correct`/`.explanation` ban đầu vì ô này không có 2 dòng đó); ô 2 parse bằng `_parseSplitAnswers()` — mỗi dòng `"<số>. <chữ cái>"`, KHÔNG dùng thứ tự dòng mà dùng đúng số thứ tự để khớp theo index (tránh lệch nếu dán thiếu/dư dòng); ô 3 parse bằng `_parseSplitExplanations()` — tách theo dòng trống giống định dạng gộp, bỏ qua số thứ tự nếu có ghi kèm. Ghép 3 phần theo index rồi `validateImportQuestion()` lại — số câu lệch giữa 3 ô thì câu thiếu đáp án/giải thích vẫn hiện ở preview với `_error` tương ứng, sửa tay được như flow dán gộp. `_importPasteMode` ('input'|'split') nhớ chế độ đang dùng để nút "← Dán lại" ở bước preview quay đúng ô vừa dán.
- **Nạp thêm câu nối tiếp ở preview**: nút "➕ Nạp thêm câu" cạnh "← Dán lại" toggle `#import-append-box` (1 textarea, luôn dùng định dạng gộp Azota dù trước đó vào preview bằng đường gộp hay tách — preview chỉ cần data đã merge sẵn nên không cần phân biệt). `appendMoreImportQuestions()` parse rồi `_importQuestions.concat(added)` — KHÔNG ghi đè danh sách đang sửa, giữ nguyên các câu đã chỉnh tay trước đó, số thứ tự câu mới tự nối tiếp theo `i+1` ở `buildImportPreviewCard()`.
